...
paid_sessions AS (
  SELECT
      ...
      s.visit_date::timestamp              AS visit_date,   -- для CSV как timestamp
...
leads_ps AS (
  SELECT
      ...
      l.status_id,
      ps.visit_ts,
      ps.visit_date,                -- timestamp (как в ожиданиях эталона)
      ps.utm_source,
      ps.utm_medium,
      ps.utm_campaign,
      ROW_NUMBER() OVER (
...
SELECT
    ps.visitor_id,
    ps.visit_date,         -- TIMESTAMP в формате 'YYYY-MM-DD HH:MI:SS.mmm'
    ps.utm_source,
    ps.utm_medium,
    ps.utm_campaign,
    ll.lead_id,
