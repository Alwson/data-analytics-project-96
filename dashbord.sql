WITH paid_sessions AS (
  SELECT
      ...
      s.visit_date::date      AS visit_date,     -- дата визита для агрегации
...
leads_agg AS (
  SELECT
      ...
      COUNT(DISTINCT CASE
          WHEN llpc.closing_reason IN ('Успешная продажа','Успешно реализовано') OR llpc.status_id = 142
          THEN llpc.lead_id
...
-- ===== Funnel (Visitors -> Leads -> Purchases) | старый Funnel Chart | step/value | все UTM =====

-- 1) Платные визиты (как в витрине)
WITH paid_sessions AS (
  SELECT
      s.visitor_id,
      s.visit_date::timestamp AS visit_ts,
...
-- 2) Кандидатные пары (lead × платный визит того же пользователя до/в момент created_at)
leads_ps AS (
  SELECT
      l.lead_id,
      l.visitor_id,
...
-- 3) Атрибуция: ровно один визит на лид (последний платный)
leads_lpc AS (
  SELECT
      lead_id,
      visitor_id,
...
-- 4) ВИЗИТЫ считаем отдельно (как в основной витрине) — без join с лидами
visits_agg AS (
  SELECT
      p.visit_date,
      p.utm_source,
...
-- 5) ЛИДЫ/ПОКУПКИ считаем отдельно по атрибутированным лидам
leads_agg AS (
  SELECT
      ll.visit_date,
      ll.utm_source,
      ll.utm_medium,
      ll.utm_campaign,
      COUNT(DISTINCT ll.lead_id) AS leads_count,
      COUNT(DISTINCT CASE
          WHEN ll.closing_reason IN ('Успешная продажа','Успешно реализовано')
             OR ll.status_id = 142
          THEN ll.lead_id END)    AS purchases_count
  FROM leads_lpc ll
  GROUP BY ll.visit_date, ll.utm_source, ll.utm_medium, ll.utm_campaign
),

-- 6) Единый набор ключей
keys AS (
  SELECT visit_date, utm_source, utm_medium, utm_campaign FROM visits_agg
  UNION
  SELECT visit_date, utm_source, utm_medium, utm_campaign FROM leads_agg
...
-- 7) Преобразуем в формат step/value под старый Funnel
steps AS (
  -- Visitors
  SELECT
      k.visit_date, k.utm_source, k.utm_medium, k.utm_campaign,
      '1. Visitors'::text AS step,
      COALESCE(v.visitors_count, 0)::numeric AS value
  FROM keys k
  LEFT JOIN visits_agg v
    ON v.visit_date   = k.visit_date
   AND v.utm_source   = k.utm_source
  ...
  SELECT
      k.visit_date, k.utm_source, k.utm_medium, k.utm_campaign,
      '2. Leads'::text AS step,
      COALESCE(l.leads_count, 0)::numeric AS value
  FROM keys k
  LEFT JOIN leads_agg l
    ON l.visit_date   = k.visit_date
   AND l.utm_source   = k.utm_source
  ...
  SELECT
      k.visit_date, k.utm_source, k.utm_medium, k.utm_campaign,
      '3. Purchases'::text AS step,
      COALESCE(l.purchases_count, 0)::numeric AS value
  FROM keys k
  LEFT JOIN leads_agg l
    ON l.visit_date   = k.visit_date
   AND l.utm_source   = k.utm_source
...
SELECT
    ...
    value
FROM steps
ORDER BY visit_date, utm_source, utm_medium, utm_campaign, step;
-- Для Funnel chart