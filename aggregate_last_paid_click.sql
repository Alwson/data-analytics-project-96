...
WITH ads_union AS (
  -- Все расходы из vk_ads и ya_ads (не ограничиваем медиум и источник)
  SELECT
...
paid_sessions AS (
  -- Все платные визиты (как в постановке): cpc, cpm, cpa, youtube, cpp, tg, social
  -- НИЧЕГО не отрезаем по наличию расходов — иначе пропадут строки вроде vk/social с нулевой cost
  SELECT
      s.visitor_id,
...
-- Визиты: считаем УНИКАЛЬНЫХ посетителей по ключу дата+utm
visits_agg AS (
  SELECT
      p.visit_date,
      p.utm_source,
...
-- Атрибуция лидов: последний платный визит (из paid_sessions) до/в момент лида
leads_ps AS (
  SELECT
      l.lead_id,
      l.created_at,
      ...
      l.closing_reason,
      l.status_id,
      ps.visit_date,     -- DATE-ключ для агрегации
      ps.utm_source,
      ps.utm_medium,
      ps.utm_campaign,
      ROW_NUMBER() OVER (
...
-- Лиды/покупки/выручка по ключу дата+utm
leads_agg AS (
  SELECT
      visit_date,
      utm_source,
...
-- Полный набор ключей, чтобы не потерять строки без расходов или без лидов
keys AS (
...
SELECT
    k.visit_date,                                -- ДОЛЖНА остаться DATE (YYYY-MM-DD)
    COALESCE(v.visitors_count, 0) AS visitors_count,
    k.utm_source,
    k.utm_medium,
    k.utm_campaign,
    COALESCE(a.total_cost, 0)     AS total_cost,         -- 0 там, где расходов нет
