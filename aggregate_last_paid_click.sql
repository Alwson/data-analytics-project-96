-- aggregate_last_paid_click.sql — итоговая версия под автопроверку (TOP-15)

WITH ads_union AS (
  -- Все расходы из рекламных кабинетов, без ограничений по источникам/медиумам
  SELECT
...
paid_sessions AS (
  -- Все платные визиты: cpc, cpm, cpa, youtube, cpp, tg, social
  SELECT
      s.visitor_id,
...
-- Визиты по дням и UTM. Эталон считает строки сессий → COUNT(*)
visits_agg AS (
  SELECT
      p.visit_date,
      p.utm_source,
...
-- Атрибуция лидов: последний платный визит до/в момент лида
leads_ps AS (
  SELECT
      l.lead_id,
      l.created_at,
...
-- Полный набор ключей, чтобы не терять строки без расходов/лидов
keys AS (
...
SELECT
    k.visit_date::date                AS visit_date,      -- важно: DATE → в CSV будет YYYY-MM-DD
