-- aggregate_last_paid_click.sql — visitors_count = LPU из лидов ∪ LPU "по дню" только для тех, у кого НЕТ лидов вообще
WITH sessions_norm AS (
  ...
  -- последний ПЛАТНЫЙ визит в каждый день по посетителю
  SELECT
    x.visitor_id,
    x.visit_date,
    x.utm_source,
  ...
  -- если у посетителя есть лид в этот день — берём метки строго из last_paid_click
  SELECT DISTINCT
    lpc.visitor_id,
...
), visitors_without_any_leads AS (
  -- "безлидовые" вообще (нет ни одного лида в датасете): берём их last paid session по дню
  SELECT lps.*
  FROM last_paid_sessions lps
  LEFT JOIN (
    SELECT DISTINCT visitor_id FROM leads
  ) any_lead ON any_lead.visitor_id = lps.visitor_id
  WHERE any_lead.visitor_id IS NULL
), visitors_all AS (
  -- объединяем: лидовые (по присвоению) ∪ полностью безлидовые (по последнему платному визиту дня)
  SELECT visitor_id, visit_date, utm_source, utm_medium, utm_campaign FROM visitors_from_leads
  UNION
  SELECT visitor_id, visit_date, utm_source, utm_medium, utm_campaign FROM visitors_without_any_leads
), visits_agg AS (
  SELECT
      v.visit_date,
      v.utm_source,
  ...
  SELECT
      ...
      COUNT(DISTINCT CASE
        ...
        THEN lpc.lead_id END) AS purchases_count,
      SUM(CASE
        WHEN lpc.closing_reason = 'Успешно реализовано' OR lpc.status_id = 142
        THEN lpc.amount ELSE NULL END) AS revenue
  FROM last_paid_click lpc
