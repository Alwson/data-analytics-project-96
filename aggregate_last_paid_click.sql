-- aggregate_last_paid_click.sql — версия под автотест (TOP-15), только yandex/vk cpc

WITH ads_union AS (
  -- Расходы только по yandex/vk и только cpc
  SELECT
...
-- Визиты по дням+UTM: именно COUNT(*), не DISTINCT
visits_agg AS (
  SELECT
      p.visit_date,
      p.utm_source,
...
-- Атрибуция лидов: последний yandex/vk cpc-визит до/в момент лида
leads_ps AS (
  SELECT
      l.lead_id,
      l.created_at,
...
-- Ключи: чтобы не потерять строки без расходов/лидов
keys AS (
...
SELECT
    k.visit_date::date                AS visit_date,      -- YYYY-MM-DD в CSV
