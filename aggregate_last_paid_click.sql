WITH last_paid_click AS (
  ...
  WHERE lower(s.medium) IN ('cpc','cpm','cpa','youtube','cpp','tg','social')
    AND lower(s.source) IN ('yandex','vk')   -- чтобы не залетали admitad/tg и пр.
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY sn.visit_ts DESC,
                 sn.utm_source DESC, sn.utm_medium DESC, sn.utm_campaign DESC,
                 sn.visitor_id DESC
      ) AS rn
    FROM sessions_norm sn
  ) x
  WHERE x.rn = 1
), visitors_from_leads AS (                   -- люди, у кого был лид (дата визита берется из LPC)
  SELECT DISTINCT
    lpc.visitor_id,
  ...
  SELECT
    ...
    v.utm_medium,
    v.utm_campaign,
    COUNT(DISTINCT v.visitor_id) AS visitors_count      -- по твоим требованиям: уникальные посетители
  FROM visitors_all v
