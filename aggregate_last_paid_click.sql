-- aggregate_last_paid_click.sql (экспорт для CSV TOP-15)
WITH ads_union AS (
  ...
  WHERE lower(utm_source) IN ('yandex','vk')      -- <-- фильтр под тест
    AND lower(utm_medium) = 'cpc'                 -- <-- фильтр под тест
...
visits_agg AS (
  SELECT
      p.visit_date, p.utm_source, p.utm_medium, p.utm_campaign,
